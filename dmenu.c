...
usage(void)
{
	fputs("usage: dmenu [-bfinrvP] [-l lines] [-h height] [-p prompt] [-fn font] [-m monitor]\n"
	      "             [-nb color] [-nf color] [-sb color] [-sf color] [-w windowid] [-pn number]\n"
	      "             [-it text]\n", stderr);
	exit(1);
}
...
main(int argc, char *argv[])
{
	...
	for (i = 1; i < argc; i++)
		...
		/* these options take one argument */
			...
			embed = argv[++i];
		else if (!strcmp(argv[i], "-pn"))   /* preselected item */
